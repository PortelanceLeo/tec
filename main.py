...
def clean_data(file):
    df = pd.read_csv(file).rename(columns=FIELD_MAPPING, errors="ignore")
    if df.empty:
        return
    ...
    # clean strings
    string_columns = np.array(
        ([f.value for f in StringFields] + [f.value[0] for f in SizedStringFields])
    )
    string_sizes = np.array(
        ([255 for f in StringFields] + [f.value[1] for f in SizedStringFields])
    )
...
def process_data(date, cycle):
    ...
    if df is not None:
        ...
        print(
            f"Loaded {count} rows into the db for {date.strftime('%m/%d/%Y')} cycle {cycle}"
        )
    else:
